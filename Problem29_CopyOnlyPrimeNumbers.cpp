 ...
 * @param Counter: A separate index for the destination array to avoid gaps
 * @return: The actual count of prime numbers found (New Size)
 */
int CopyOnlyPrimeNumbers(int Arr[100],int Arr2[100],int Size,int Size2){
    ...
    for (int i = 0; i < Size; i++)
    {
        ...
        {
            Arr2[Counter]=Arr[i];
            Counter++;// Only increment when a prime is actually added
        }
...
int main(){
    ...
    // Step 2: Filter primes into Arr2 and capture the new size
    // We pass Size to know where to stop in Arr, but Size2 will tell us where Arr2 ends
    int Size2=CopyOnlyPrimeNumbers(Arr,Arr2,Size,Size2);
    // Step 3: Display the filtered array using the new size (Size2)
