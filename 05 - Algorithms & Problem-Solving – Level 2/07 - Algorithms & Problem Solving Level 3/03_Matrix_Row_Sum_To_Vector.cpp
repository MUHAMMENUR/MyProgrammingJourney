...
void SumOfRaw(int matrix[3][3],vector <int>&SumVector) {
    ...
    for (int i = 0; i < 3; i++) {
        ...
        SumVector.push_back(RowSum);
     
    }
    }

/**
 * Prints the calculated row sums stored in the vector using pointer arithmetic.
 *
 * @param SumVector Constant reference to the vector containing the row sums.
 */

void PrintSumOfRow(const vector<int>&SumVector) {
    ...
    for(auto &sum: SumVector) {
        ...
        cout << "This is the sum of row " << &sum - &SumVector[0] + 1 << ": " << sum << endl;
    }
    cout << endl;
}
