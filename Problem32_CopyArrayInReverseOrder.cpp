...
/**
 * Copies elements in reverse using a single loop and a mathematical formula
 * Formula: DestinationIndex [i] = SourceIndex [Size - 1 - i]
 */
...
void CopyArrayInReverseOrder(int Arr[100],int Arr2[100],int Size){
    ...
    for (int i = 0; i<Size; i++)
    {
        // We calculate the reverse index directly
       Arr2[i]=Arr[Size-1-i];
    }
